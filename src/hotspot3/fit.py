...
class BackgroundFit:
    ...
    def get_signal_bins(self, array: np.ndarray, min_bg_tr=None):
        ...
        result = np.full((n_signal_bins, min_bg_tr.shape[0]), np.nan, dtype=np.float32)
    ...
    def get_all_bins(self, array: np.ndarray):
        ...
        bg_bins = np.full((n_bg_bins, min_bg_tr.shape[0]), np.nan)
...
class GlobalBackgroundFit(BackgroundFit):
    ...
    def estimate_global_mean_and_var(self, agg_cutcounts: np.ndarray):
        has_enough_background = agg_cutcounts.size > 0 & (np.count_nonzero(agg_cutcounts) / agg_cutcounts.size > self.config.nonzero_windows_to_fit)
